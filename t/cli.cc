...
void check_good_arguments()
{
	...
	if (opt.paths_in_out.size() != 1 || opt.paths_in_out.front().first != "x" ||
	    opt.paths_in_out.front().second != "y" || !opt.delete_all || opt.overwrite ||
	...
	if (opt.paths_in_out.size() != 1 || opt.paths_in_out.front().first != "x" ||
	    opt.paths_in_out.front().second != "x" || !opt.overwrite || opt.to_delete.size() != 0 ||
	...
	if (opt.paths_in_out.size() != 3 || opt.paths_in_out[0].first != "x" || opt.paths_in_out[0].second != "x" ||
	    opt.paths_in_out[1].first != "y" || opt.paths_in_out[1].second != "y" ||
	    opt.paths_in_out[2].first != "z" || opt.paths_in_out[2].second != "z" || !opt.overwrite)
